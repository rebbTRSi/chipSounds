...
int onemS = 0;
int bitDepth = 16;
float samplesPerMinute;
float sampleIntervalQuarter;
...
void initialize(int sampleRate, int bitDepth)
{
	...
	duration = 10.0;
	int currentCalculated = 0;
	samplesPerMinute = SR*60.0;
	sampleIntervalQuarter= samplesPerMinute/(float)pattern.bpm;
...
struct epattern getCurrentPattern() {
	return pattern;
}
...
void changeCurrentParameter(int changInstr, int parameterNr,int parameterTo){
	if (parameterNr >= 0 && parameterNr <= 16) {
		//  printf ("currentInstrument: %d, parameter: %d , value: %d\n",changInstr,parameterNr,parameterTo);
		//   printf ("changing: %d\n",instrumentsArray[changInstr].wave[parameterNr]);
		instrumentsArray[changInstr].wave[parameterNr] = parameterTo;
		//   printf ("to: %d\n",instrumentsArray[changInstr].wave[parameterNr]);
...
void setBpm(int newBpm) {
	pattern.bpm = newBpm;
	beatsPerSecond = (float)newBpm/60.0f;
	quarterNoteLengths = (1000.0f/beatsPerSecond);
	sixteenthNoteLength = quarterNoteLengths / 4.0f; // ms
	barLenghtInMs = quarterNoteLengths * 4.0f;
	quarternNotesPerSecond = 1000.0f*quarterNoteLengths;
