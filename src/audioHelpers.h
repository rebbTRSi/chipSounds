...
double calcSine (double f) { sines += f * cosine; cosine -= f*sines; return sines; }
double sineCoefficient (int FRuency, int SR) { return 2.0 * M_PI * FRuency / SR; }
double sineOsc (int A,int FRuency,int SR) { double coef=sineCoefficient(FRuency,SR); return A*calcSine(coef); }     
...
double noiseOsc (int A) { return  A*((2.0*((rand()%(127- (-128)))+ (-128))-1.0)/127); }
